...
class WarningController extends Controller
{
    public function getMyWarnings(Request $request) {
        ...
        if($property) {
            ...
            // Procurar unidade com esse id e desse proprietário:
            ...
            if($unit > 0) {
                ...
                foreach($warnings as $warnKey => $warnValue) { 
                    $warnings[warnKey]['datecreated'] = date('d/m/Y', strtotime($warnValue['datecreated'])); // ajustar a hora certinha.
                    $photoList = [];
    ...
    public function setWarning(Request $request) {
        ...
        if(!$validator->fails()) {
            ...
            if($list && is_array($list)) { //array com as fotos para montar no banco de dados.
                $photos = [];
